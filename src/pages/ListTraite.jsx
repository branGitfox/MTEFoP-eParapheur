...
function ListTraite() {
  ...
  const fetchByDirection = async () => {
    setLoader(true)
    try{
      await axiosRequest.get(`/serviceToSp?page=${page}&lines=${lines}`, {headers:{Authorization:`Bearer ${token}`}})
    ...
    return (
      ...
      {doc.type!="recuperation" && (
        ...
        <td className="px-4 py-3 text-sm">
          ...
          <button onClick={() =>changeLivreStatus(doc.m_id)} disabled={doc.status =='non reçu'?false:true} className={` ${doc.status =='non reçu'?'bg-green-500':'bg-gray-300'} px-3 py-2  text-gray-50 rounded-2xl`}>
            {livreLoader?(     <Oval
              visible={true}
              height="15"
              width="15"
